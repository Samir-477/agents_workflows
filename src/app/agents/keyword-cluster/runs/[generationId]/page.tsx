...
export default async function KeywordClusterRunPage({ params }: { params: Promise<{ generationId: string }> }) {
  ...
  return <main className="mx-auto max-w-[1280px] px-5 py-8 sm:px-8 sm:py-12"><nav aria-label="Breadcrumb" className="mb-8 text-sm text-[#777680]"><Link href="/agents" className="hover:text-[#e94320]">Agents</Link><span className="mx-2">›</span><Link href="/agents/keyword-cluster" className="hover:text-[#e94320]">Keyword Cluster Agent</Link><span className="mx-2">›</span><span>Run</span></nav><KeywordClusterRun generationId={generationId} /></main>;
}
