...
export function SessionIntelligenceView({ report, onOpenAgent, onRefresh, busy }: { report: ManagementReport; onOpenAgent: (agent: string) => void; onRefresh: () => void; busy: boolean }) {
  ...
  const handoffs = intelligence.recommended_agent_handoffs || [];
  const rawBuckets = [
  ...
  return <div>
    <Section eyebrow="Executive intelligence" title="Decision brief" copy="A concise, evidence-bound view of the current position and the next management decision.">
      <div className="overflow-hidden rounded-[22px] bg-[#102d22] text-white shadow-[0_16px_38px_rgba(15,45,34,.12)] lg:grid lg:grid-cols-[1.35fr_.65fr]">
        <div className="p-6 sm:p-8">
          ...
          <h3 className="mt-3 max-w-3xl text-2xl font-semibold leading-8 tracking-[-.035em]">{top ? top.action : handoffs[0]?.title || "No implementation action was established in this scope."}</h3>
          <p className="mt-4 max-w-3xl text-sm leading-6 text-white/70">{top ? top.rationale : handoffs[0]?.reason || "Keep the assessment as a dated baseline and rerun after material evidence changes."}</p>
    ...
    <Section eyebrow="Coverage" title="What this session assessed" copy="Scores reflect only the agents that participated. Unassessed engines remain outside the conclusion.">
      <div className="grid gap-4 md:grid-cols-3">
        {intelligence.engine_scores.map(item => { const style = engineStyle[item.engine]; const confidence=item.score_kind==="evidence_confidence"; return <button key={item.engine} onClick={() => onOpenAgent(item.weakest_agent)} className="rounded-[20px] border border-[#dce2df] bg-white p-5 text-left shadow-[0_9px_24px_rgba(23,31,27,.05)] transition hover:-translate-y-0.5"><div className="flex items-center justify-between"><span className="flex items-center gap-2 text-sm font-semibold"><i className={`h-2.5 w-2.5 rounded-full ${style.dot}`} />{item.engine} {confidence?"evidence":"Engine"}</span><span className="text-[11px] text-[#77817d]">{item.agent_count} agent{item.agent_count === 1 ? "" : "s"}</span></div><div className="mt-5 flex items-end justify-between"><div><b className="text-4xl tracking-[-.05em]">{item.score}</b><span className="ml-1 text-xs text-[#7b8581]">/100</span></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${style.soft} ${style.text}`}>{confidence?"Evidence confidence":item.score_status.label}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-[#e8ecea]" role="progressbar" aria-label={`${item.engine} ${confidence?"evidence confidence":"engine score"}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={item.score}><div className={`h-full rounded-full ${style.bar}`} style={{ width: `${item.score}%` }} /></div><dl className="mt-5 space-y-2 border-t pt-4 text-xs"><div className="flex justify-between gap-3"><dt className="text-[#77817d]">{confidence?"Interpretation":"Critical / warnings"}</dt><dd className="font-semibold">{confidence?"Not a readiness score":`${item.critical_count} / ${item.warning_count}`}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#77817d]">Review first</dt><dd className="truncate font-semibold">{item.weakest_agent_label}</dd></div></dl></button>; })}
    ...
    {handoffs.length ? <Section eyebrow="Connected analysis" title="Recommended agent handoffs" copy="These are diagnostic next steps. They are kept outside the implementation queue until a downstream agent verifies a website change."><div className="grid gap-4 md:grid-cols-2">{handoffs.map((item,index)=><button key={`${item.source_agent}-${item.target_agent}-${index}`} onClick={()=>item.source_agent&&onOpenAgent(item.source_agent)} className="rounded-[20px] border border-violet-200 bg-violet-50/50 p-5 text-left"><div className="flex items-center justify-between gap-4"><span className="text-[10px] font-bold uppercase tracking-[.14em] text-violet-700">{item.source_agent_label} → {item.target_label}</span><span className="rounded-full bg-white px-2.5 py-1 text-[10px]">{item.question_ids.length} questions</span></div><h3 className="mt-3 font-semibold">{item.title}</h3><p className="mt-2 text-xs leading-5 text-[#626c68]">{item.reason}</p><span className="mt-4 block text-xs font-semibold text-violet-700">Review discovery evidence →</span></button>)}</div></Section> : null}
